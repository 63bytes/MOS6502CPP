...
private:
	...
	using OPCODE_FUNCTION = void(mos6502old::*)(INSTRUCTION_OPHAND);
	struct Opcode {
...
mos6502old::mos6502() {
	...
	OPCODEDATA[0x69] = Opcode(&mos6502old::OP_ADC, 0x69, IMM, 2, 2);
	OPCODEDATA[0x6D] = Opcode(&mos6502old::OP_ADC, 0x6D, ABS, 4, 3);
	OPCODEDATA[0x65] = Opcode(&mos6502old::OP_ADC, 0x65, ZP, 3, 2);
	OPCODEDATA[0x61] = Opcode(&mos6502old::OP_ADC, 0x61, INDX, 6, 2);
	OPCODEDATA[0x71] = Opcode(&mos6502old::OP_ADC, 0x71, INDY, 5, 2);
	OPCODEDATA[0x75] = Opcode(&mos6502old::OP_ADC, 0x75, ZPX, 4, 2);
	OPCODEDATA[0x7D] = Opcode(&mos6502old::OP_ADC, 0x7D, ABX, 4, 3);
	OPCODEDATA[0x79] = Opcode(&mos6502old::OP_ADC, 0x79, ABY, 4, 3);
}
}